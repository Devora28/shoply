...
const userVote = computed(() => {
  return props.review?.votes?.[0] ?? null
})
</script>
      ...
      <div class="flex-1">
        ...
        <div class="flex items-center gap-4 mt-3">
          <button
            ...
            :class="userVote?.is_helpful === true ? 'text-primary-600' : 'text-ink-500 hover:text-ink-900'"
            @click="$emit('vote',true)"
          >
            <ThumbsUp class="w-3.5 h-3.5" /> {{ review?.helpful_count }}
          </button>
          <button
            class="inline-flex items-center gap-1.5 text-xs font-medium transition-colors"
            :class="userVote?.is_helpful === false ? 'text-danger-600' : 'text-ink-500 hover:text-ink-900'"
        ...
        <div v-if="review.seller_reply" class="mt-3 ml-4 pl-4 border-l-2 border-ink-200">
          <div class="flex items-center gap-2 mb-1">
            ...
            <span class="text-xs text-ink-400">{{ dayjs(review.seller_replied_at).format('MMM D, YYYY') }}</span>
          </div>
